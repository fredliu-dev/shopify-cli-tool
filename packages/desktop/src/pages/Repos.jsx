...
function Masonry({
  ...
  const measure = useCallback(() => {
    ...
    setLayout({
      ...
      heights,
      colWidth,
      height: Math.max(0, Math.max(...colHeights) - gap),
    });
  }, [minColWidth, gap]);
  ...
  const onPointerMove = (e) => {
    ...
    if (pend) {
      ...
      setDrag({
        itemIndex: pend.itemIndex,
        pointerId: pend.pointerId,
        offX: pend.offX,
        offY: pend.offY,
        curLeft: pend.cardLeft,
        curTop: pend.cardTop,
      });
      return;
    }
    ...
    setDrag((d) =>
      d ? { ...d, curLeft: mx - d.offX, curTop: my - d.offY } : d,
    );
...
function SaveRepoModal({
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      window.api.repos
        .templates()
        .then((r) => setTplOptions(r.ok ? r.data : []));
      if (repo?.remoteUrl) {
    ...
    Promise.all([
      window.api.repos.resolveTemplate(dev.store),
      window.api.repos.templates(),
    ]).then(([r1, r2]) => {
  ...
  const findDup = (vals) => {
    ...
    return (projects || []).find(
      (p) =>
        ...
        String(p.previewKey ?? "").trim() ===
          String(vals.preview_key ?? "").trim() &&
  ...
  const dup = useMemo(() => {
    ...
    if (!String(wTheme ?? "").trim() || !String(wDesc ?? "").trim())
      return null;
    return findDup({
      theme: wTheme,
      preview_key: wPreviewKey,
      project_desc: wDesc,
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
  ...
  return (
    <Modal
      title={`${isNew ? "初始化并保存" : "本地保存"} - ${repo?.name ?? ""}`}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
    >
      <Form
        ...
        layout='vertical'
        onFinish={submit}
        initialValues={isNew ? { port: "9292" } : undefined}
      >
        {!isNew && (
          <>
        ...
        {(isNew || resolvedTpl === null) && (
          ...
          >
            <Select
              options={tplOptions.map((t) => ({ label: t, value: t }))}
              placeholder='选择模板'
            />
          </Form.Item>
        )}
          ...
          <Space.Compact style={{ width: "100%" }}>
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "请输入 theme（或点右侧复制 live 主题）",
                },
              ]}
            >
          ...
          <Form.Item
            ...
            extra={
              <Text type='secondary' style={{ fontSize: 12 }}>
                如 /pages/back-to-school-sale；无 preview_key
                时拼到预览/开发链接，编辑器链接挂 previewPath 参数
              </Text>
            }
          >
        ...
        <Form.Item
          name='project_desc'
          label='project_desc（标题，可附工单链接）'
          rules={[{ required: true, message: "请输入 project_desc" }]}
        >
        ...
        <Space>
          ...
          {dup && (
            ...
            >
              <QuestionCircleOutlined
                style={{ color: "#faad14", fontSize: 16, cursor: "help" }}
              />
            </Tooltip>
          )}
        </Space>
      ...
      >
        <Form form={copyForm} layout='vertical' onFinish={doCopyLive}>
          ...
          >
            ...
            <WorkItemSelect
              titleOnly
              footerHint='手填活动名即可；选工单会自动用工单标题填充'
            />
          </Form.Item>
          <Form.Item
            name='owner'
          ...
          >
            <AutoComplete
              ...
              filterOption={(v, o) =>
                String(o.value).toLowerCase().includes(String(v).toLowerCase())
              }
              style={{ width: "100%" }}
            >
          ...
          <Text type='secondary' style={{ display: "block", marginBottom: 12 }}>
            主题名格式：[dev] 活动 | 负责人 | 日期；需该 store 已 shopify
            login。
          </Text>
...
function ChangedJsonModal({ open, title, files, onClose }) {
  return (
    <Modal
      title={title ? `JSON改动 - ${title}` : "JSON改动"}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
...
function ThemeListModal({ open, repo, onClose, onChanged }) {
  ...
  const editorLinkOf = (id) =>
    `https://admin.shopify.com/store/${store.split(".")[0]}/themes/${id}/editor`;
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            ...
            <span>
              目标店铺：
              <b style={{ color: "rgba(255,255,255,0.95)" }}>{store}</b>
            </span>
          </div>
          <Text type='secondary' style={{ fontSize: 12, lineHeight: "20px" }}>
            {live ? (
              <>
                发布后对访客立即生效，替换当前 live「{live.name}」（#{live.id}
                ）；原 live 主题不会被删除。
              </>
            ) : (
  ...
  const confirmDelete = (t) => {
    modal.confirm({
      ...
      onOk: async () => {
        setBusyId(t.id);
        const r = await window.api.repos.deleteTheme({
          dir,
          themeId: t.id,
          store,
        });
...
function SettingsModal({ open, defaultEditor, onClose, onSaved }) {
  ...
  return (
    ...
    >
      {editors.length === 0 ? (
        <Text type='secondary'>
          未检测到本机已装的编辑器（VS Code / Cursor / WebStorm 等）。
        </Text>
      ) : (
        <Radio.Group
          value={value}
...
function CreateProjectModal({
  ...
  const submit = async () => {
    ...
    for (const t of picks) {
      const res = await window.api.repos.clone({
        workspaceDir,
        github: t.github,
      });
    ...
    if (!failed.length) message.success(`已克隆 ${results.length} 个项目`);
      ...
      message.error(
        `${results.length - failed.length} 成功；${failed.length} 失败：${failed.map((f) => `${f.name}(${f.error})`).join("、")}`,
      );
    onDone?.();
  };
      ...
      >
        <Form.Item
          ...
          rules={[
            ...
            {
              pattern: /^[A-Za-z0-9_-]+$/,
              message: "仅字母、数字、下划线和中划线（将作为文件名）",
            },
          ]}
          extra={
            <Text type='secondary' style={{ fontSize: 12 }}>
              将作为文件名，新建后在所有「选模板」处可见
            </Text>
          }
        >
...
function ContactEditModal({ open, contact, onClose, onDone }) {
  ...
  const submit = async (vals) => {
    setLoading(true);
    const res = await window.api.contacts.upsert({
      id: contact?.id,
      name: vals.name,
      phone: vals.phone,
    });
...
function GroupEditModal({ open, group, onClose, onDone }) {
  ...
  const submit = async (vals) => {
    setLoading(true);
    const res = await window.api.dingtalk.upsertGroup({
      id: group?.id,
      ...vals,
    });
    setLoading(false);
...
function TemplateEditModal({ open, template, onClose, onDone }) {
  ...
  const submit = async (vals) => {
    ...
    const res = await window.api.dingtalk.upsertTemplate({
      id: template?.id,
      name: vals.name,
  ...
  return (
    <Modal
      title={template?.id ? "编辑信息模板" : "新增信息模板"}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
    ...
    >
      <Form form={form} layout='vertical' onFinish={submit}>
        ...
        <Form.Item
          ...
          extra={
            <Text type='secondary' style={{ fontSize: 12 }}>
              占位符：<Text code>{"{{@person as 姓名}}"}</Text>{" "}
              <Text code>{"{{@url}}"}</Text> <Text code>{"{{@title}}"}</Text>{" "}
              <Text code>{"{{@content as 备注}}"}</Text>{" "}
              <Text code>{"{{@tapd as 工单}}"}</Text>
              <Text code>{"{{@all}}"}</Text>；多行直接换行。
            </Text>
          }
        >
          <TextArea
...
function DingtalkTemplatesModal({ open, onClose, onChange }) {
  ...
  const handleClearDefaults = async (id) => {
    const res = await window.api.dingtalk.saveDefaults({
      templateId: id,
      defaults: {},
    });
    if (res.ok) {
  ...
  const columns = [
    { title: "模板名称", dataIndex: "name", key: "name" },
    {
      ...
      ellipsis: true,
      render: (c) => (
        <Text style={{ fontSize: 12 }}>{(c || "").replace(/\n/g, " ")}</Text>
      ),
    },
    {
      title: "默认负责人",
      ...
      render: (_, t) => {
        ...
        return (
          <Text style={{ fontSize: 12 }}>{Object.values(d).join("，")}</Text>
        );
      },
    },
    {
        ...
        <Space size={6} wrap>
          ...
          {t.defaults && Object.keys(t.defaults).length > 0 && (
            <Popconfirm
              ...
              okText='清除'
              cancelText='取消'
              onConfirm={() => handleClearDefaults(t.id)}
            >
...
function GotestModal({ open, project, projects, contacts, onClose }) {
  ...
  const splitPhone = (raw) => {
    ...
    return m
      ? { phone: m[1].trim(), display: m[2].trim() }
      : { phone: String(raw).trim(), display: String(raw).trim() };
  };
  ...
  const onTemplateChange = async (id) => {
    ...
    const personFields = fs.filter(
      (f) => f.kind === "person" && defaults[f.token],
    );
    if (personFields.length) {
      const preview = personFields
        .map(
          (f) =>
            `${f.label}：${phoneDisplay(splitPhone(defaults[f.token]).phone)}`,
        )
        .join("\n");
      modal.confirm({
        title: "检测到默认负责人，是否使用？",
        ...
        onOk: () =>
          setValues((s) => {
            const next = { ...s };
            personFields.forEach(
              (f) => (next[f.token] = splitPhone(defaults[f.token]).phone),
            );
            return next;
          }),
      });
  ...
  const submit = async () => {
    ...
    const badPhone = fields.find(
      (f) => f.kind === "person" && !/^1\d{10}$/.test(values[f.token]),
    );
    if (badPhone) {
      message.warning(
        `「${badPhone.label}」的手机号 ${values[badPhone.token]} 不是 11 位，钉钉无法 @ 到人，请先在「人员管理」修正`,
      );
      return;
    }
    setLoading(true);
    const res = await window.api.dingtalk.gotest({
      groupId,
      templateId,
      values,
    });
    ...
    if (res.ok) {
      ...
      const defaults =
        templates.find((t) => t.id === templateId)?.defaults || {};
      const picked = {};
      fields
        ...
        .forEach((f) => {
          ...
          const sameAsDefault =
            cur != null && splitPhone(cur).phone === values[f.token];
      ...
      if (Object.keys(picked).length) {
        const preview = Object.entries(picked)
          .map(
            ([tok, ph]) =>
              `${fields.find((f) => f.token === tok)?.label || tok}：${phoneDisplay(ph)}`,
          )
          .join("\n");
        modal.confirm({
          ...
          cancelText: "不保存",
          onOk: async () => {
            const r = await window.api.dingtalk.saveDefaults({
              templateId,
              defaults: picked,
            });
  ...
  const PLACEHOLDER_RE =
    /\{\{\s*@(person|url|title|content|tapd|all)(\d*)\s*(?:as\s+(.+?))?\s*\}\}/g;
  const rendered = (() => {
    ...
    >
      <Form layout='vertical'>
        <Form.Item label='本地项目' required>
          <Select
            ...
            onChange={(id) => {
              ...
              if (fields.length)
                setValues((s) => ({ ...s, ...applyProject(p, fields) }));
            }}
            options={(projects || []).map((p) => ({
              value: p.id,
              label: p.description || p.store,
            }))}
        ...
        <Form.Item label='消息模板' required>
          <Select
            placeholder={
              noTemplates ? "请先在「信息模板管理」添加模板" : "选择消息模板"
            }
            value={templateId}
          ...
          >
            {f.kind === "person" ? (
              ...
              <TextArea
                ...
                value={values[f.token] || ""}
                onChange={(e) =>
                  setValues((s) => ({ ...s, [f.token]: e.target.value }))
                }
              />
            ) : (
              <Input
...
function githubTreeUrl(remoteUrl, branch) {
  ...
  return /^https?:\/\//.test(u)
    ? `${u}/tree/${encodeURIComponent(branch)}`
    : null;
}
...
function syncMessage(sync, prefix) {
  ...
  if (sync.applied) {
    const name =
      sync.project?.description ||
      sync.project?.templateName ||
      sync.project?.store ||
      "";
    return {
      type: "success",
      text: `${prefix}${name ? `，已套用项目「${name}」配置` : ""}`,
    };
  }
  if (sync.reason === "template-missing") {
    return {
      type: "warning",
      text: `${prefix}，项目引用的模板「${sync.templateName}」已删除，配置未切换`,
    };
  }
  if (sync.reason === "no-project" && sync.hadToml) {
    return {
      type: "info",
      text: `${prefix}，该分支无本地项目，已删除 shopify.theme.toml`,
    };
  }
  return null;
}
...
function MergeInfoModal({ open, repo, projects, contacts, onClose }) {
  ...
  const {
    local: branchesLocal,
    remote: branchesRemote,
    loading: branchLoading,
    reload: reloadBranches,
  } = useRepoBranches(repo);
  ...
  const rendered = (() => {
    if (!tpl || !selected.length)
      return { text: "", atMobiles: [], isAtAll: false };
  ...
  const atMobiles =
    customPreview == null
      ? rendered.atMobiles
      : rendered.atMobiles.filter((p) => preview.includes(`@${p}`));
  ...
  const step1Ready =
    ...
    !!templateId &&
    !fields.some((f) => f.kind === "person" && !values[f.token]);
  ...
  const doSend = async () => {
    ...
    setLoading(true);
    const res = await window.api.dingtalk.notify({
      groupId,
      text: preview,
      atMobiles,
      isAtAll: rendered.isAtAll,
    });
    setLoading(false);
    if (res.ok) {
      const g = groups.find((x) => x.id === groupId);
  ...
  const doCreatePr = async () => {
    if (!repo?.currentBranch)
      return message.warning("当前仓库未检测到分支（PR head）");
    ...
    if (!res.ok) {
      const netHint =
        /fetch|ENETUNREACH|ETIMEDOUT|ECONNRESET|getaddrinfo|network|网络/i.test(
          res.error || "",
        )
          ? "（请确认已开启 VPN）"
          : "";
      message.error({
        content: `创建 PR 失败：${res.error}${netHint}`,
        duration: 8,
      });
      return;
    }
    const { url, reviewerWarning, reviewerFailed, reviewerError } =
      res.data || {};
        ...
        <div>
          {reviewerWarning && (
            <div style={{ marginBottom: 8 }}>
              <Text type='warning'>
                部分 Reviewer 请求失败
                {Array.isArray(reviewerFailed) && reviewerFailed.length
                  ? `（未能添加：${reviewerFailed.join("、")}）`
                  : ""}
                {reviewerError ? `，原因：${reviewerError}` : ""}，请到 GitHub
                手动补加。
              </Text>
            </div>
          )}
  ...
  const inputFields = fields.filter(
    (f) => f.kind === "person" || f.kind === "content",
  );

  return (
    <Modal
      title={`获取合并提交信息 - ${repo?.name || ""}`}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
      width={620}
      // 内容区限高滚动：字段多时弹窗不再超出屏幕
        ...
        {step === 1 && (
          <>
            ...
            <Form.Item
              ...
              extra={
                <Text type='secondary' style={{ fontSize: 12 }}>
                  <Text type='warning'>⚠️ 需开启 VPN</Text> 才能访问 GitHub
                  拉取仓库成员，否则会拉取失败；成员可留空（非必填）。
                </Text>
              }
            >
              {needToken ? (
                <div>
                  <Text style={{ display: "block", marginBottom: 8 }}>
                    拉取仓库成员需要一个 <Text strong>GitHub Token</Text>
                    （仅本机自用，明文存于本地配置）。
                  </Text>
                  <Input.Password
                    value={token}
                  ...
                  >
                    ...
                    <br />
                    2. Note 随便填（如「Shopify Toolbox」）；
                    <Text strong>
                      勾选 <Text code>repo</Text>
                    </Text>
                    （私有仓读权限）；
                    <br />
                    3. 拉到底点 <Text code>Generate token</Text>，复制{" "}
                    <Text code>ghp_...</Text> 粘贴到上方。
                    <br />
                    4. 若保存后仍报 404，说明该 Token 账号不是仓库协作者，或
                    Token 未勾选 repo 权限。
                    <br />
                    <br />
                    <Text type='warning'>
                      ⚠️ Token
                      等同于账号密码，请勿分享/提交到仓库。若保存后仍提示
                      404，请确认该 Token 对应账号已是仓库协作者。
                    </Text>
                  </Text>
                </div>
              ) : (
        ...
        {step === 0 && (
          <>
            ...
            >
              ...
              <Tooltip
                title={
                  step1Ready ? "" : "请先选择项目、模板并填写人员，再进入下一步"
                }
              >
                <span>
                  <Button
            ...
            >
              ...
              <Form.Item label='Commit 类型'>
                <Select
                  ...
                  options={COMMIT_TYPES.map((t) => ({
                    value: t.value,
                    label: `${t.value}（${t.desc}）`,
                  }))}
                />
              </Form.Item>
              <Form.Item
                label='PR 标题（左：选项目自动填充；右：可手改）'
                required
              >
                <Space.Compact style={{ width: "100%" }}>
                  <Select
              ...
              >
                <Text type='secondary' style={{ fontSize: 12 }}>
                  ...
                  <Text code>
                    {prTitle.trim()
                      ? formatCommitTitle(prType, prTitle)
                      : `${prType}: ...`}
                  </Text>
                </Text>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <Tooltip
                  title={needToken ? "创建 PR 需先配置 GitHub Token" : ""}
                >
                  <span>
                    <Button
                      type='primary'
...
/* ---------------- 批量提测（顶部工具栏入口：多选本地项目 + 模板 + 群，合并成一条消息发送） ----------------
 * 「相同信息只填一次」：
 * 1) 表单层——url/title/tapd 若所有选中项目取值完全一致（如同一工单下的多个项目），表单只展示一份
 *    可编辑输入并自动预填；取值不一致的字段不进表单，渲染时按各项目自身信息填充。
 * 2) 消息层——行级合并：模板中含「各项目取值不一致」占位符的行（标题/工单/测试链接）每项目渲染一份，
 *    其余行（【提测通知】抬头、开发/测试/经理、备注——各项目完全相同）整条消息只渲染一次。
 */
function BatchGotestModal({ open, projects, contacts, onClose }) {
  const { message, modal } = App.useApp();
  const [groups, setGroups] = useState([]);
  const [templates, setTemplates] = useState([]);
  const [groupId, setGroupId] = useState();
  const [templateId, setTemplateId] = useState();
  const [fields, setFields] = useState([]); // parsePlaceholders 返回的字段列表
  const [values, setValues] = useState({}); // 共用字段的手填值（person/content 手输；共用 url/title/tapd 可改写预填）
  const [selectedIds, setSelectedIds] = useState([]);
  const [parsing, setParsing] = useState(false);
  const [loading, setLoading] = useState(false);
  ...
  // 自动填充字段的项目取值口径（与单项目提测/合并信息一致）
  const autoValueOf = (kind, p) => {
    if (kind === "url") return p?.links?.previewLink || "";
    if (kind === "title") return p?.description || "";
    if (kind === "tapd") return p?._tapd || "";
    return "";
  };

  // 兼容 CLI「手机号（姓名）」存法：拆出纯手机号；并按手机号从 contacts 反查展示名（与单项目提测一致）
  const splitPhone = (raw) => {
    const m = String(raw).match(/^(.+?)\s*[（(](.+?)[）)]\s*$/);
    return m
      ? { phone: m[1].trim(), display: m[2].trim() }
  ...
  // 相同信息判定：自动字段在所有选中项目取值一致且非空 → 视为共用（token -> 共同值）
  const sharedAuto = (() => {
    const map = {};
    if (!selected.length) return map;
    fields.forEach((f) => {
      if (f.kind !== "url" && f.kind !== "title" && f.kind !== "tapd") return;
      const first = autoValueOf(f.kind, selected[0]);
      if (!first) return;
      if (selected.every((p) => autoValueOf(f.kind, p) === first))
        map[f.token] = first;
    });
    return map;
  })();

  // 选完模板：解析占位符（person/content 共用一份；url/title/tapd 依「是否所有项目相同」决定进表单或按项目自动填）；
  // 模板存了 @person 默认值时询问是否使用（与单项目提测一致）
  const onTemplateChange = async (id) => {
    setTemplateId(id);
    if (!id) {
      setFields([]);
    ...
    if (personFields.length) {
      ...
      modal.confirm({
        title: "检测到默认负责人，是否使用？",
        content: <Text style={{ whiteSpace: "pre-wrap" }}>{previewTxt}</Text>,
        okText: "使用默认",
        cancelText: "不用",
        onOk: () =>
          setValues((s) => {
  ...
  const PLACEHOLDER_RE =
    /\{\{\s*@(person|url|title|content|tapd|all)(\d*)\s*(?:as\s+(.+?))?\s*\}\}/g;
  // 渲染单行：person/content 与共用 url/title/tapd 用表单值（只填一次），
  // 非共用的 url/title/tapd 按项目各自填充；@all 由钉钉处理
  const renderLine = (line, p) => {
    const atMobiles = [];
    let isAtAll = false;
    const text = String(line).replace(PLACEHOLDER_RE, (_full, type, num) => {
      const token = `@${type}${num ?? ""}`;
      if (type === "person") {
        const phone = (values?.[token] ?? "").trim();
        if (phone) atMobiles.push(phone);
      ...
      if (type === "content") return (values?.[token] ?? "").trim();
      if (sharedAuto[token] != null)
        return (values?.[token] ?? sharedAuto[token]).trim();
      return autoValueOf(type, p);
    });
    return { text, atMobiles, isAtAll };
  };
  // 行是否随项目变化：含「各项目取值不一致」的自动占位符（url/title/tapd）→ 每个项目渲染一份
  const isVaryingLine = (line) => {
    let varying = false;
    String(line).replace(PLACEHOLDER_RE, (_full, type, num) => {
      if (
        (type === "url" || type === "title" || type === "tapd") &&
        sharedAuto[`@${type}${num ?? ""}`] == null
      )
        varying = true;
      return "";
    });
  ...
  // 行级合并：模板按行拆分，连续的「随项目变化」行作为一组，组内按项目渲染（项目间空行分隔）；
  // 其余行（如【提测通知】抬头、开发/测试/经理、备注——各项目完全相同）整条消息只渲染一次。
  const rendered = (() => {
    if (!tpl || !selected.length)
      return { text: "", atMobiles: [], isAtAll: false };
    ...
    while (i < lines.length) {
      if (!isVaryingLine(lines[i])) {
        const r = renderLine(lines[i], selected[0]);
        atMobiles.push(...r.atMobiles);
        isAtAll = isAtAll || r.isAtAll;
        out.push(r.text);
        i++;
      } else {
        let j = i;
        while (j < lines.length && isVaryingLine(lines[j])) j++;
        const groupLines = lines.slice(i, j);
        selected.forEach((p, idx) => {
          if (idx > 0) out.push(""); // 项目块之间空行分隔
          groupLines.forEach((ln) => {
            const r = renderLine(ln, p);
            atMobiles.push(...r.atMobiles);
            isAtAll = isAtAll || r.isAtAll;
            out.push(r.text);
          });
    ...
    return {
      text: out.join("\n"),
      atMobiles: [...new Set(atMobiles)],
      isAtAll,
    };
  })();
  // 手动改过预览时以改后文本为准；合成来源变化（改项目/模板/人员）时丢弃手动修改
  const preview = customPreview ?? rendered.text;
  const atMobiles =
    customPreview == null
      ? rendered.atMobiles
  ...
  const doSend = async () => {
    if (!selectedIds.length) return message.warning("请选择至少一个本地项目");
    if (!templateId) return message.warning("请选择消息模板");
    if (!groupId) return message.warning("请选择通知群");
    const missing = fields.find((f) => f.kind === "person" && !values[f.token]);
    if (missing) return message.warning(`请为「${missing.label}」选择人员`);
    // person 手机号须为 11 位：钉钉只 @ 群成员的真实手机号（与单项目提测同口径）
    const badPhone = fields.find(
      (f) => f.kind === "person" && !/^1\d{10}$/.test(values[f.token] || ""),
    );
    if (badPhone) {
    ...
    if (res.ok) {
      const g = groups.find((x) => x.id === groupId);
      message.success(`已发送到「${g?.name || "群"}」`);
      // 发送成功后询问是否把本次 person 存为默认值（与单项目提测一致）；
      // 与现有默认值（按手机号比对）相同的不再提示——存了也是重复
      const defaults =
        templates.find((t) => t.id === templateId)?.defaults || {};
      const picked = {};
      ...
      if (Object.keys(picked).length) {
        const previewTxt = Object.entries(picked)
          .map(
            ([tok, ph]) =>
              `${fields.find((f) => f.token === tok)?.label || tok}：${phoneDisplay(ph)}`,
          )
  ...
  return (
    <Modal
      title={`批量提测（已选 ${selected.length} 个项目）`}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
    ...
    >
      <Form layout='vertical'>
        ...
        {fields.map((f) => {
          const isSharedAuto =
            (f.kind === "url" || f.kind === "title" || f.kind === "tapd") &&
            sharedAuto[f.token] != null;
          // 非共用的 url/title/tapd：各项目取值不同，渲染时自动按项目填充，不在表单重复展示
          if (!isSharedAuto && f.kind !== "person" && f.kind !== "content")
            return null;
          return (
            <Form.Item
              key={f.token}
            ...
            >
              {f.kind === "person" ? (
                ...
                <Input
                  value={values[f.token] ?? sharedAuto[f.token]}
                  onChange={(e) =>
                    setValues((s) => ({ ...s, [f.token]: e.target.value }))
                  }
                />
        ...
        <Form.Item
          label='通知内容预览（相同信息合并，变化信息按项目逐份填充，可直接修改）'
          extra={
            customPreview != null && rendered.text ? (
              <ALink
                style={{ fontSize: 12 }}
        ...
        >
          <TextArea
            ...
            onChange={(e) => setCustomPreview(e.target.value)}
            autoSize={{ minRows: 4, maxRows: 12 }}
            placeholder='选择项目、模板后在此预览，可手动修改后再复制/发送'
          />
        </Form.Item>
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <Button onClick={doCopy} disabled={!preview}>
...
/* ---------------- 网址后台打开（仓库卡片级：输入路由，按当前配置 store/theme 拼编辑器链接跳转） ----------------
 * 拼接与本地项目的「编辑器」链接完全同款（links.js editorLink：
 * https://admin.shopify.com/store/<store>/themes/<theme>/editor?previewPath=<encodeURIComponent 路径>），
 * 只是路由由用户输入：贴完整网址会自动剔除域名只留路径作 previewPath。
 */
function AdminOpenModal({ open, repo, onClose }) {
  const { message } = App.useApp();
  const [url, setUrl] = useState("");
  const [jumping, setJumping] = useState(false);

  const store = repo?.devEnv?.store || "";
  const theme = String(repo?.devEnv?.theme || "").trim();

  useEffect(() => {
    if (open) setUrl("");
  }, [open]);

  // 解析输入网址：剔除域名只留路径（normalizePreviewPath 同款口径：去首尾斜杠后补单个前导斜杠）。
  // 三种写法分口径处理：带协议完整 URL → 直接解析；裸域名（首段含点如 shokz.com）→ 补协议解析；
  // 纯路径（/pages/xx、pages/xx）→ 直接当 previewPath，不进 URL 解析
  // （注意：https:///pages/xx 这种补出来的三斜杠 URL 会把 pages 当主机名，故纯路径绝不能走 URL 解析）。
  // query 里若带 preview_theme_id 一并取出（仅作展示，主题以当前配置为准）。
  const parsed = (() => {
    const raw = String(url || "").trim();
    if (!raw) return { path: "", urlThemeId: "" };
    const extract = (u) => ({
      path:
        u.pathname && u.pathname !== "/"
          ? "/" + u.pathname.replace(/^\/+|\/+$/g, "")
          : "",
      urlThemeId: u.searchParams.get("preview_theme_id") || "",
    });
    // 带协议头的完整网址
    if (/^[a-z][a-z0-9+.-]*:\/\//i.test(raw)) {
      try {
        return extract(new URL(raw));
      } catch {
        return { path: "", urlThemeId: "" };
      }
    }
    // 裸域名：首个片段含点（如 shokz.com/pages/xx），补协议再解析
    const [beforeQuery, query] = raw.split("?");
    const firstSeg = beforeQuery.split("/")[0];
    const themeIdOf = (q) =>
      q ? new URLSearchParams(q).get("preview_theme_id") || "" : "";
    if (firstSeg.includes(".")) {
      try {
        return extract(new URL(`https://${raw}`));
      } catch {
        return { path: "", urlThemeId: "" };
      }
    }
    // 纯路径：/pages/xx 或 pages/xx（可带 ?query）
    const clean = beforeQuery.replace(/^\/+|\/+$/g, "");
    return { path: clean ? `/${clean}` : "", urlThemeId: themeIdOf(query) };
  })();

  const storePrefix = store.split(".")[0];
  // 与 links.js editorLink 完全同款：当前配置的 store + theme，previewPath 用输入的路由
  const adminUrl =
    storePrefix && theme && parsed.path
      ? `https://admin.shopify.com/store/${storePrefix}/themes/${theme}/editor?previewPath=${encodeURIComponent(parsed.path)}`
      : "";
  const noTheme = !!storePrefix && !theme;

  const doJump = async () => {
    setJumping(true);
    const res = await window.api.shell.openExternal(adminUrl);
    setJumping(false);
    if (res?.ok) message.success("已在默认浏览器打开 Shopify 后台");
    else message.error("打开链接失败");
  };

  return (
    <Modal
      title={`网址后台打开 - ${repo?.name || ""}`}
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
    ...
    >
      <Form layout='vertical'>
        ...
        <Form.Item
          ...
          extra={
            <Text type='secondary' style={{ fontSize: 12 }}>
              贴完整网址即可，自动剔除域名只留路径作 previewPath（如
              https://shokz.com/pages/xx → /pages/xx）
            </Text>
          }
        >
          <Input
            ...
            placeholder='https://shokz.com/pages/back-to-school-sale'
            allowClear
          />
        </Form.Item>
        {!!url.trim() && (
          <Form.Item label='解析结果'>
            ...
            >
              {parsed.path || "（未解析到路径）"}
              {parsed.urlThemeId
                ? `｜网址自带主题 ID：${parsed.urlThemeId}（不采用，以当前配置 theme 为准）`
                : ""}
            </Text>
          </Form.Item>
        )}
        <Form.Item label='后台跳转地址预览'>
          <TextArea
            value={adminUrl}
            readOnly
    ...
    >
      ...
      >
        <Form.Item
          name='base'
          label='基准分支'
          rules={[{ required: true, message: "请选择基准分支" }]}
        >
          <Select
            showSearch
            loading={branchLoading}
        ...
        <Text type='secondary' style={{ display: "block", marginBottom: 12 }}>
          分支名预览：
          <Text code>{branchName || "dev/{type}-{负责人}-{编号}"}</Text>
        </Text>
        <Button type='primary' htmlType='submit' loading={loading}>
          创建分支
        </Button>
...
function OpenEditorButton({ dir, defaultEditor }) {
  ...
  const onClick = async () => {
    // 有默认编辑器：一键打开，不展开菜单
    if (defaultEditor) {
      const res = await window.api.repos.openInEditor({
        dir,
...
function RepoCard({
  ...
  const adminLink = storeName
    ? `https://admin.shopify.com/store/${storeName}/themes`
    : null;
  // GitHub 当前分支链接。
  ...
  const openDepGraph = async () => {
    const res = await window.api.repos.openDepGraph({
      dir: repo.path,
      name: repo.name,
    });
    if (!res.ok) message.error(res.error || "打开失败");
  ...
  const askDeleteBranch = () => {
    ...
    if (branch === "main" || branch === "master")
      return message.warning("主分支不允许删除");
      ...
      content: (
        <div>
          ...
          <Text type='secondary' style={{ fontSize: 12, display: "block" }}>
            store {p.store} · theme {String(p.theme ?? "-")} ·{" "}
            {p.description || "无描述"}
          </Text>
          <Text
            type='warning'
          ...
          >
            选「删除主题」会先从 Shopify
            删除该线上主题并清理引用它的本地项目，然后再进入删除分支确认；
            选「跳过」则不动主题，直接删除分支。
          </Text>
        </div>
      ),
  ...
  const deleteThemeThenAsk = (project, branch) =>
    new Promise((resolve) => {
      ...
      if (!themeId) {
        message.warning(
          "项目缺少 theme 字段，跳过删主题，直接进入删除分支确认",
        );
      ...
      (async () => {
        ...
        if (!res.data) {
          message.warning(
            `主题 ${themeId} 线上已不存在，跳过删主题，直接进入删除分支确认`,
          );
          return resolve(openDelBranchModal());
        }
        if (res.data.role === "live") {
          message.error(
            "live 主题不允许删除，已终止；可重新点击「删除分支」选「跳过」",
          );
          return resolve();
        }
        modal.confirm({
          ...
          content: (
            <div>
              ...
              >
                ...
                <Descriptions.Item label='theme ID'>
                  {themeId}
                </Descriptions.Item>
  ...
  const branchDeletable =
    !!repo.currentBranch &&
    !["main", "master", "alt"].includes(repo.currentBranch);
  ...
  return (
    ...
    >
      {/* 仓库名称标签行：普通文档流，完全突出在卡片上方，不遮挡任何元素。
          卡片自身无上边框，顶边线由标签右缘接续画出，标签与卡片之间没有分割线 */}
      <div style={{ display: "flex", alignItems: "flex-end" }}>
          ...
          <div
            style={{
              ...
              borderColor: hovered
                ...
                : "rgba(255,255,255,0.1)",
              boxShadow: `inset 0 1px 0 rgba(255,255,255,${
                hovered ? 0.22 : 0.08
              })`,
              color: "#fff",
              fontSize: 15,
        ...
        />
        {/* GitHub 小旗：立在卡片右上角上方（文档流内，不遮挡卡片内容），旗面向右飘，点击跳转当前分支 GitHub 页 */}
        {githubUrl && (
            ...
            >
              ...
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  background: "#69b1ff",
                  boxShadow: "0 0 6px rgba(105,177,255,0.8)",
                }}
              />
            </div>
          </Tooltip>
      ...
      <Card
        ...
        style={{
          ...
          borderTop: "none", // 顶边线由上方标签行接续画出，避免标签与卡片之间出现分割线
          borderRadius: "0 12px 12px 12px", // 左上角直角，与标签连成一体
          transition:
            "background .25s ease, border-color .25s ease, box-shadow .25s ease",
          transform: undefined,
        }}
      >
        <div style={{ position: "relative" }}>
          ...
          >
            <Tooltip title='切换分支（展开自动拉取最新分支列表）'>
              <div
                style={{
                  flex: 1,
                  minWidth: 0,
                  ...
                  borderRadius: 999,
                  background:
                    "linear-gradient(135deg, rgba(105,177,255,0.65) 0%, rgba(22,119,255,0.25) 45%, rgba(255,255,255,0.14) 100%)",
                  boxShadow:
                    "0 2px 12px rgba(22,119,255,0.22), inset 0 1px 0 rgba(255,255,255,0.12)",
                }}
              >
                <Select
                  size='small'
          ...
          >
            <Tooltip title='点击复制完整路径'>
              <div
                className='path-chip'
                onClick={async () => {
                  const res = await window.api.shell.copy(repo.path);
                  if (res?.ok) message.success("路径已复制");
                }}
              >
                <FolderOpenOutlined
                  style={{ color: "#69b1ff", fontSize: 12, flexShrink: 0 }}
                />
                <span className='path-chip-text'>{repo.path}</span>
              </div>
            </Tooltip>
              ...
              >
                ...
                >
                  <AppstoreOutlined
                    ...
                    style={{ color: "#b37feb" }}
                  />
                  <span className='tile-label'>主题列表</span>
                </div>
              </Tooltip>
          ...
          {projects.map((p) => (
            <ProjectPanel
              ...
              embedded
              themeProjectCount={
                themeProjectCounts?.get(
                  `${p.store}|${String(p.theme ?? "").trim()}`,
                ) || 0
              }
            />
          ))}
        ...
        <Modal
          ...
          onOk={doDeleteBranch}
          okText={
            delCountdown > 0 ? `请确认信息（${delCountdown}s）` : "确认删除"
          }
          okButtonProps={{
            danger: true,
            disabled: delCountdown > 0 || delBranchLoading,
        ...
        >
          ...
          >
            ...
            <Descriptions.Item label='远程分支'>
              {delBranchModal?.remoteExists ? (
                <Text type='danger'>存在，将一并删除</Text>
              ) : (
                <Text type='secondary'>不存在（仅删本地）</Text>
              )}
            </Descriptions.Item>
            <Descriptions.Item label='关联本地项目'>
              {delBranchModal?.projectCount > 0 ? (
                <Text type='warning'>
                  {delBranchModal.projectCount} 个（线上主题是否已删见上方操作）
                </Text>
              ) : (
          ...
          {delBranchModal?.branch === repo.currentBranch && (
            <Text type='warning' style={{ fontSize: 12, display: "block" }}>
              ⚠️ 删除的是当前所在分支，删除前会自动切换到 main / master /
              其他本地分支。
            </Text>
          )}
        </Modal>
...
function EditProjectModal({ open, project, onClose, onDone }) {
  ...
  const editableKeys = (Object.keys(project || {}) || [])
    ...
    .filter(
      (k) => !k.startsWith("_") && !EDIT_SKIP.has(k) && k !== "previewPath",
    )
    .sort((a, b) => {
  ...
  useEffect(() => {
    if (open && project) {
      ...
      vals._tapd = project._tapd
        ? { title: "", url: String(project._tapd) }
        : null;
  ...
  const submit = async (vals) => {
    ...
    const res = await window.api.shops.update(
      project.id,
      payload,
      project.repoPath,
    );
        ...
        {editableKeys.map((k) =>
          EDIT_READONLY.has(k) ? (
            <Form.Item key={k} label={EDIT_LABELS[k] || k}>
              <Input
                value={project?.[k] != null ? String(project[k]) : ""}
                disabled
              />
            </Form.Item>
          ) : (
...
function ProjectPanel({
  ...
  const askDeleteTheme = async () => {
    ...
    if (!project.repoPath)
      return message.warning("未找到所属仓库，无法删除主题");
      ...
      content: (
        <div>
          ...
          {themeProjectCount > 0 && (
            ...
            >
              ⚠️ 将同时删除引用该主题的 {themeProjectCount}{" "}
              条本地项目记录（可能含其他分支的项目）。
            </Text>
          )}
          {active && (
            ...
            >
              ⚠️
              该主题正在当前生效配置中使用，删除后将一并清除该仓库的生效配置。
            </Text>
          )}
        </div>
      ),
  ...
  const ACTIVE_GLOW = active
    ? {
        ...
        boxShadow:
          "0 0 0 1px rgba(82,196,26,0.3), 0 8px 22px rgba(82,196,26,0.3)",
        position: "relative",
        overflow: "hidden",
      }
    : {};
  ...
  const title =
    project.description || project.templateName || project.store || "-";
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        {active && (
          <div
            ...
            style={{
              ...
              pointerEvents: "none",
              background:
                "linear-gradient(110deg, transparent 25%, rgba(82,196,26,0.42) 50%, transparent 75%)",
        ...
        >
          ...
          {project.templateName && (
            <Tag className='glass-tag'>{project.templateName}</Tag>
          )}
          {active && <Tag className='glass-tag glass-tag-green'>当前生效</Tag>}
          {noRepo && (
        ...
        >
          <Space size={6}>
            <Badge
              count={project.changedJson?.length || 0}
              size='small'
              offset={[-2, 0]}
              color={project.changedJson?.length ? "#faad14" : undefined}
            >
              <Button
                size='small'
                className='glass-btn'
                onClick={() =>
                  onAction("json", { title, files: project.changedJson || [] })
                }
              >
                JSON改动
              </Button>
          ...
          <Space size={6}>
            <Tooltip title='用 shopify 删除该 theme ID 对应的线上主题，并连带清理引用它的本地项目'>
              <Button
                ...
                loading={themeDelLoading}
                onClick={askDeleteTheme}
              >
                删除主题
              </Button>
            </Tooltip>
            <Popconfirm
              ...
              okText='删除'
              cancelText='取消'
              onConfirm={() => onAction("delete", project)}
            >
              <Button
                size='small'
                danger
      ...
      {tapdUrl && (
        <TapdItemDrawer
          open={tapdDrawerOpen}
          link={tapdUrl}
          onClose={() => setTapdDrawerOpen(false)}
        />
      )}
    </div>
  );
...
function AboutModal({ open, onClose, onOpenReleases }) {
  ...
  const rows = [
    ...
    [
      "系统 Node",
      info?.systemNode?.fallback
        ? "未找到（shopify 功能不可用）"
        : info?.systemNode?.path,
    ],
  ];
  return (
    <Modal
      title='关于 Shopify 工具箱'
      open={open}
      onCancel={onClose}
      footer={null}
      destroyOnClose
...
const GITHUB_RELEASES_URL =
  "https://github.com/fredliu-dev/shopify-cli-tool/releases/latest";
...
export default function Repos({ registerMenu }) {
  ...
  const [batchGotestOpen, setBatchGotestOpen] = useState(false); // 批量提测（多选本地项目）
  const [adminOpenFor, setAdminOpenFor] = useState(null); // 网址后台打开目标 repo（卡片级入口）
  ...
  const refreshCloneable = useCallback(
    async (dir) => {
  ...
  const refreshRepo = async (repoPath) => {
    ...
    if (res.ok)
      setRepos((prev) => prev.map((r) => (r.path === repoPath ? res.data : r)));
    await refreshProjects();
  };
  ...
  const repoAction = (type, payload) => {
    ...
    else if (type === "adminOpen") setAdminOpenFor(payload);
  };
  ...
  const handleDeleteProject = async (project) => {
    ...
    message.success(
      res.synced ? "已删除项目，并清除该仓库当前生效配置" : "已删除",
    );
  ...
  const handleSwitch = async (project) => {
    ...
    const res = await window.api.repos.switchConfig({
      dir: project.repoPath,
    ...
    if (data.applied) {
      ...
      if (port?.wasOccupied) {
        if (port.killed > 0) text += `，已释放被占用的端口 ${port.port}`;
          ...
          message.warning(
            `端口 ${port.port} 被占用且未能结束进程，请手动处理后重试`,
          );
      }
      message.success(text);
    } else if (data.skipped === "tracked") {
      message.warning(
        "shopify.theme.toml 被 Git 跟踪，无法自动切换（请先在 .gitignore 忽略该文件）",
      );
    } else if (data.reason === "template-missing") {
      message.warning(
        `项目引用的模板「${data.templateName}」已删除，配置未切换`,
      );
    } else {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Text
                strong
                ellipsis={{ tooltip: workspaceDir }}
                style={{ maxWidth: 340, minWidth: 0 }}
              >
                {workspaceDir}
              </Text>
              <Text style={{ fontSize: 12, color: "#69b1ff", flexShrink: 0 }}>
                {repos.length} 个仓库
              </Text>
            </div>
          </div>
        ...
        <Space size={8} style={{ flexShrink: 0 }}>
          ...
          >
            <span>
              <Button
                type='primary'
                icon={<PlusOutlined />}
                disabled={!cloneable.some((t) => !t.exists)}
          ...
          <Button
            ...
            onClick={() => setBatchGotestOpen(true)}
          >
            批量提测
          </Button>
          ...
          description={
            <Space direction='vertical' size={4}>
              <Text style={{ fontSize: 12 }}>
                主题复制 / 删除 / 信息等 shopify 功能依赖系统
                Node.js（≥22）运行，当前机器未找到，相关操作会失败。安装后重启本应用即可。
              </Text>
        ...
        <Masonry minColWidth={440} gap={12} draggable onReorder={handleReorder}>
          {repos.map((r) => (
            <RepoCard
              ...
              branchProjectCounts={
                branchProjectCountsByStore.get(r.devEnv?.store) || {}
              }
      ...
      <GotestModal
        ...
        projects={
          gotestFor
            ? projectsByRepoPath.get(gotestFor.repoPath) || [gotestFor]
            : []
        }
        contacts={contacts}
      ...
      {/* 批量提测（跨仓库多选本地项目，相同信息只填一次，合并成一条消息发送） */}
      <BatchGotestModal
        open={batchGotestOpen}
        projects={enrichedProjects}
        contacts={contacts}
        onClose={() => setBatchGotestOpen(false)}
      />

      {/* 网址后台打开（贴网址剔域名作 previewPath，按卡片当前配置 store/theme 拼编辑器链接跳转） */}
      <AdminOpenModal
        open={!!adminOpenFor}
        repo={adminOpenFor}
        onClose={() => setAdminOpenFor(null)}
      />
      ...
      <MergeInfoModal
        ...
        projects={
          mergeInfoFor
            ? (projectsByRepoPath.get(mergeInfoFor.path) || []).filter(
                (p) => p.description,
              )
            : []
        }
