import { APP_NAME, detectPlatform, downloadAsset, fetchLatestRelease, openFile, pickAsset } from './_desktop.js'
...
export default {
    ...
    async run({ log }) {
        ...
        if (!release) {
            spin.fail(`还没有发布版本，请稍后再试或联系开发者。也可前往发布页手动下载：${pc.cyan('https://github.com/fredliu-dev/shopify-cli-tool/releases')}`)
            return 1
        }
